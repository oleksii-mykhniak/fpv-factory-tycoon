...
//
// Кадрів менше, ніж віддає генератор (36), і це не обрізання заради ваги.
// 36 кадрів — не цикл: аркуш містить два кроки з хвостиком, тож останній кадр
// не сходився з першим, і хода смикалася раз на оберт. scripts/analyze-ai-sheet.js
// знаходить вікно, де стик такий самий м'який, як рух усередині; лишилися саме
// ці вікна. Файли схудли з 1.37 МБ до 484 КБ — це наслідок, а не мета.
export const AI_SHEETS = Object.freeze({
  idle:   { key: 'player_idle',    frames: 13, frameMs: 90 },
  // Айдл спиною. Персонаж, що спинився дорогою вгору, лишається спиною до
  // глядача: розворот «до камери» на кожній зупинці читається як окремий рух,
  // якого гравець не просив, і збиває відчуття, що це той самий крок.
  idleUp: { key: 'player_idle_up', frames: 17, frameMs: 90 },
  down:   { key: 'player_walk_down', frames: 16, frameMs: 42 },
  up:     { key: 'player_walk_up',   frames: 13, frameMs: 42 },
  side:   { key: 'player_walk_side', frames: 13, frameMs: 42 },
  // Хода від глядача з предметом у руках. Є лише цей напрямок: убік і вниз
  // предмет лишається над головою, бо старі аркуші намальовані з опущеними
  // руками й чіпляти до них коробку нема за що.
  upCarry: { key: 'player_walk_up_carry', frames: 19, frameMs: 42 },
})

// Де сидить предмет, який персонаж несе В РУКАХ, — частка висоти спрайта ВНИЗ
// від центру актора. Це рівень пояса, де фігура найвужча.
//
// Предмет малюється ЗА персонажем: він далі від камери, ніж спина того, хто
// його несе. Тому з-за фігури видно лише краї — і саме це читається як «несе
// перед собою». Спроба покласти предмет ПОВЕРХ тулуба провалилася двічі:
// голова тут більша за півфігури, тулуб вужчий за коробку, і коробка на ньому
// накриває людину цілком, ким би вона не була — хоч на грудях, хоч на поясі.
//
// Висота критична. Вище (на рівні голови) фігура ширша й ховає предмет майже
// повністю; нижче предмет вилазить з-під ніг і читається як ящик, у якому
// персонаж стоїть.
//
// Масштабу тут немає навмисне: предмет у руках того самого розміру, що й на
// підлозі. Зменшувати його довелося, лише поки він лежав ПОВЕРХ тулуба.
export const CARRY_IN_HANDS_Y = 0.20

// Наскільки предмет зсунутий УПЕРЕД ПО ХОДУ в профіль — частка ширини спрайта.
// У профіль руки виставлені вбік від осі фігури, і предмет, залишений на осі,
// читається як такий, що стирчить із живота, а не як той, що його несуть.
export const CARRY_IN_HANDS_SIDE_X = 0.18

// І висота стопки НАД ГОЛОВОЮ — там, де пози немає (риг найманих робітників).
// Було зашите 0.55 просто в sceneSync: єдине число розміщення, яке лишалось
// поза config.js, і тому єдине, яке не можна було підкрутити, не шукаючи його.
export const CARRY_OVER_HEAD_Y = 0.55
...
// ── Стрілка цілі (C7.3) ──────────────────────────────────
// Розмір завезеного арту (41×64) у пропорції, а не на око: розтягнута на чужий
// прямокутник стрілка тупішає вістрям, і саме вістря — єдине, заради чого вона
...
export const KIT_CONFIGS = Object.freeze({
  ...
  //
  // Кожен входить у гру через КІМНАТУ, а не через «збери 25 попередніх». Це і є
  // правило, яке робить нові типи не гриндом: щоб продавати літак, треба мати
  // де його обліт́увати, а не зібрати ще партію квадрокоптерів.
  fixedwing_drone: {
    cost: 340, basePrice: 620, deliveryMs: 12000,
    assemblySteps: [
      { label: 'Клею крило',                     missMsg: 'Шов розходиться — переклеюємо' },
      { label: 'Ставлю сервоприводи елевонів',   missMsg: 'Хід сервоприводу заїдає — переставляємо' },
      { label: 'Монтую штовхаючий мотор',        missMsg: 'Вал бʼє — перетягуємо кріплення' },
      { label: 'Паяю ESC і живлення',            missMsg: "Холодне зʼєднання — переплавляємо" },
      { label: 'Ставлю трубку Піто',             missMsg: 'Трубка підсмоктує повітря — переклеюємо' },
      { label: 'Прошиваю автопілот',             missMsg: 'Автопілот не бачить сенсор — перевіряємо шину' },
    ],
  },
  heavy_drone: {
    cost: 620, basePrice: 1180, deliveryMs: 16000,
    assemblySteps: [
      { label: 'Збираю раму гексакоптера',       missMsg: 'Промінь не сідає в замок — перезбираємо' },
      { label: 'Ставлю шість моторів',           missMsg: 'Один мотор виходить із фази — перевіряємо пайку' },
      { label: 'Паяю силову шину',               missMsg: 'Шина гріється — переплавляємо' },
      { label: 'Монтую підвіс вантажу',          missMsg: 'Підвіс хитається — підтягуємо' },
      { label: 'Ставлю резервне живлення',       missMsg: 'Резерв не перемикається — перевіряємо реле' },
      { label: 'Калібрую компас',                missMsg: 'Компас ловить наводку — відносимо від шини' },
      { label: 'Тестую вантаж на землі',         missMsg: 'Перекос під вагою — переставляємо підвіс' },
    ],
  },
  proto_drone: {
    cost: 460, basePrice: 980, deliveryMs: 14000,
    assemblySteps: [
      { label: 'Ріжу раму під нову схему',       missMsg: 'Розмір не сходиться — переміряємо' },
      { label: 'Паяю дослідну плату',            missMsg: 'Доріжка відходить — переплавляємо' },
      { label: 'Ставлю сенсор на кронштейн',     missMsg: 'Кронштейн резонує — переставляємо' },
      { label: 'Тягну кабелі назовні',           missMsg: 'Кабель труть пропелери — перекладаємо' },
      { label: 'Заливаю експериментальну прошивку', missMsg: 'Прошивка не стартує — відкочуємо' },
      { label: 'Знімаю телеметрію',              missMsg: 'Телеметрія рветься — міняємо канал' },
    ],
  },
  scrap_drone: {
...
// Скільки дронів ЦЬОГО типу треба зібрати, щоб відкрити перехід на наступний
// Mk (Стадія 11 / A2). Індекс — поточний Mk.
//
// Рахуються саме ЗБІРКИ, а не продажі: продаж — це кур'єр і скринька, тобто
// штат, а Mk про те, що цех уміє робити цей дрон руками. Тому лічильник росте
// на верстаку, включно з дроном, який потім ніхто не поніс продавати.
export const MK_BUILD_REQ = [3, 6, 10, 15, 25]
...
// ── Контракти (Стадія 14 / К3) ───────────────────────────
//
// Продаж був безликий: дрон іде в ящик відвантаження, гроші капають, і немає
// жодної причини зібрати саме ЦЕЙ тип саме зараз. Контракт — це причина.
//
// Три одночасно: одного замало (тоді це не вибір, а розклад), п'ять — це вже
// список справ, у який ніхто не дивиться.
export const CONTRACT_SLOTS = 3

// Скільки штук просить контракт. Росте з репутацією: перший — «здай три»,
// пізній — партія. Асимптоти тут не треба, бо репутація сама росте повільно.
export const CONTRACT_QTY_BASE  = 3
export const CONTRACT_QTY_STEP  = 0.6

// Скільки часу дається. Рахується від нормального темпу цеху, а не від
// абсолютного числа: контракт має тиснути, а не вбивати.
export const CONTRACT_MS_PER_UNIT = 95_000
export const CONTRACT_MS_FLOOR    = 240_000

// Премія за виконаний контракт — частка від базової ціни партії. Це НЕ заміна
// виторгу: дрони й далі продаються як завжди, премія лягає зверху. Тому й
// «жодної нової дії для гравця» (К3.3): змінюється тільки ціна.
export const CONTRACT_BONUS_SHARE = 0.45

// Репутація контрактів. Штраф м'який і в тій самій валюті, що й нагорода:
// тайкун, у якому дедлайн може загнати в мінус, перестає бути грою про ріст
// (К3.4). Грошима провал не коштує НІЧОГО.
export const CONTRACT_REP_REWARD  = 2
export const CONTRACT_REP_PENALTY = 1

// ── Обліт (Стадія 14 / К4) ───────────────────────────────
//
// Єдине місце, де гра про дрони показує ДРОН У ПОЛЬОТІ, а не коробку на столі.
// Механічно — один актор по кривій; по грі — перше вузьке місце, яке не
// лікується купівлею ще одного верстака (К4.3: один майданчик, один дрон).
export const FLIGHT_MS = 4600

// Наскільки дорожчий облітаний дрон. Це плата за довший маршрут: продавець
// несе дрон спершу сюди й лише потім у скриньку.
export const FLIGHT_PRICE_BONUS = 0.18

// Нижче цієї якості дрон вважається бракованим. Обліт його ЛОВИТЬ — тобто
// брак перестає доїжджати до клієнта. Саме тут трек «Оснастка» (якість пайки)
// вперше стає видимим: що вища оснастка, то рідше майданчик щось відхиляє.
export const FLIGHT_REJECT_QUALITY = 0.42

// Скільки коштує брак, який ПРОПУСТИЛИ повз майданчик (обліт можна не робити —
// дешевше й ризикованіше). Ціна — репутація, не гроші: та сама валюта, у якій
// штрафує зірваний контракт, і з тієї самої причини (К3.4).
export const FLIGHT_SKIP_REP_PENALTY = 1
