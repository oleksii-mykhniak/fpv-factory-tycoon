...
describe('sim/intentSystem', () => {
  ...
  // Стадія 17 / двір. Довга перешкода вздовж усієї межі світу: персонаж, що
  // стоїть до неї впритул і рушає ПАРАЛЕЛЬНО їй, не має нікуди зникати.
  //
  // Виштовхування ставило його на край коробки, і для коробки завдовжки в
  // пів-світу цей край лежав за півсвіту: у дворі персонаж біля паркана,
  // натиснувши вбік, опинявся в протилежному кутку.
  it('не жбурляє на протилежний край довгої перешкоди при русі вздовж неї', () => {
    const fence = rect(500, 300, 1000, 41)     // вся нижня межа, y 279.5..320.5
    const w = world({ x: 100, y: 279.5 - 10, vx: 200, obstacles: [fence] })
    moveSystem(w, 1000)
    expect(agent(w).x).toBeCloseTo(300)        // пройшов свої 200 уздовж
    expect(agent(w).y).toBeCloseTo(269.5)      // і лишився на місці по Y
  })

  // Той самий дотик, але крок УПОПЕРЕК: сюди виштовхування якраз потрібне.
  it('і все ж спиняє того, хто в цю перешкоду йде', () => {
    const fence = rect(500, 300, 1000, 41)
    const w = world({ x: 100, y: 200, vy: 200, obstacles: [fence] })
    moveSystem(w, 1000)
    expect(agent(w).y).toBeCloseTo(269.5)
  })

  // Дотик — не перекриття. Без допуску в `overlaps` персонаж, поставлений
  // рівно на межу, прилипав до неї: сусідня секція паркана читалась як
  // перекрита, і крок уздовж відкочувався щокадру.
  it('не прилипає до перешкоди, до якої стоїть упритул', () => {
    const a = rect(100, 300, 74, 41)
    const b = rect(174, 300, 74, 41)           // сусідня секція, впритул
    const w = world({ x: 60, y: 279.5 - 10, vx: 100, obstacles: [a, b] })
    moveSystem(w, 1000)
    expect(agent(w).x).toBeCloseTo(160)
  })
